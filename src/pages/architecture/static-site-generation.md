...
A useful way to frame the split between static and dynamic content is [Islands Architecture](https://jasonformat.com/islands-architecture/): a mostly static page with self-contained interactive “islands” that load and run independently. OpenMFE microfrontends are a natural fit for those islands, since each one is a stand-alone component with its own contract, embedded into an otherwise static page and initialised on its own. The distinction worth keeping is that an island is usually scoped to a single page, whereas a microfrontend can be shared across many pages. The older, broader term [JAMstack](https://en.wikipedia.org/wiki/Jamstack) named the same static-first, API-enriched delivery model, while Islands Architecture focuses specifically on how interactive parts sit within an otherwise static page.
