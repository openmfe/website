...
If you go for client-side composition, then the choice between picking a framework and going #frameworkless is basically between accepting some degree of overhead versus being deliberately lightweight, which potentially implies more effort and less out-of-the box features. A compromise might be a lightweight framework such as Lit which ultimately gives you custom elements with just a marginal overhead.

Using more heaviweight solutions like React, Vue or Angular may result in unnecessary huge artefacts; they are not well-suited for client-side microfrontends. TypeScript is a capable language, but the JavaScript its toolchains emit tends to be larger than a hand-written equivalent, carrying assumptions and boilerplate that consume memory and are hard to debug at runtime without additional instrumentation. JSX is convenient, yet it too adds overhead to the artefact. Build tools are often configured for broad browser support, including browsers that are long obsolete, which produces a good deal of unnecessary transpilation code. And all that excess code does not only add volume that needs to be transmitted over the wire, but also consumes CPU and memory.
...
Anyway, remember: never put developer experience ahead of user experience. Choosing a heavy framework because it is the one you know, or because it has a nice editor plugin, trades your users’ experience for your own convenience.
